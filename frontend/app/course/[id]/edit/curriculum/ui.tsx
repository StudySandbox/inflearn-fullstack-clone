...
const UI = ({ initialCourse }: Props) => {
  ...
  // 섹션 삭제
  const deletedSectionMutation = useMutation({
        ...
        <div key={section.id} className="rounded-lg border bg-white p-4">
          ...
          <div className="mt-4 space-y-2">
            {section.lectures?.map((lecture: Lecture, lectureIdx: number) => (
              <div
                key={lecture.id}
                className="flex items-center justify-between rounded-md border bg-white px-2 py-2"
              >
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {lecture.isPreview ? (
                      <LockOpenIcon className="text-green-600" size={18} />
                    ) : (
