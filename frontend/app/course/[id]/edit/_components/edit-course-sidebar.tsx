...
export default function EditCourseSidebar() {
  ...
  return (
    <Card className="flex w-60 flex-col gap-8 rounded-lg border-none bg-white p-6 shadow-none">
      {steps.map((section) => (
        <div key={section.section}>
          <div className="mb-4 text-lg font-bold">{section.section}</div>
                ...
                <li key={item.key} className="flex items-start gap-3">
                  <Link
                    ...
                    className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 transition-colors ${
                      isActive
                        ? "border border-green-500 bg-green-50 font-bold text-black"
                        : ""
